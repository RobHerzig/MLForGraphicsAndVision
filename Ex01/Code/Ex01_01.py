...
def kdtree_search(tree, queries):
    start_time = datetime.datetime.utcnow()
    for q in queries:
        dist, ind = tree.query([q], k=1) #WE ONLY WANT THE NEAREST NEIGHBOUR -> k=1
...
if __name__ == '__main__':
    ...
    if test_no_kd:
        for D in range(1, 500, 10):
            ...
            time = exhaustive_search_with_queries(random_points,
                                                  random_points)  # just in case we want to use other points
